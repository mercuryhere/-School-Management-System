...
class TeacherSystem:
    ...
    def create_dashboard(self, parent):
        ...
        self.db.cursor.execute(
            "SELECT name, subject FROM teachers WHERE teacher_id = ?", 
            (self.teacher_id,)
        )
        teacher_name, subject = self.db.cursor.fetchone()
        dashboard.title(f"Teacher Dashboard - {teacher_name} ({subject})")
        dashboard.geometry("1200x700")
        
        notebook = ttk.Notebook(dashboard)
        notebook.pack(fill="both", expand=True, padx=10, pady=10)
        
        self._create_attendance_tab(notebook, subject)
        
        return dashboard
    
    def _create_attendance_tab(self, notebook, subject):
